...
public class MnemonicUtils {
    ...
    static {
        for (Bip39Locale l : Bip39Locale.values()) {
            WORD_LIST.put(l, Collections.unmodifiableList(populateWordList(l.getLocale())));
        }
    }
    ...
    private static int mnemonicToBits(String mnemonic, BitSet bits) {
        return mnemonicToBits(Bip39Locale.ENGLISH, mnemonic, bits);
    }

    private static int mnemonicToBits(Bip39Locale lang, String mnemonic, BitSet bits) {
        int bit = 0;
        final StringTokenizer tokenizer = new StringTokenizer(mnemonic, " ");
        while (tokenizer.hasMoreTokens()) {
            final String word = tokenizer.nextToken();
            final int index = WORD_LIST.get(lang).indexOf(word);
            if (index < 0) {
                throw new IllegalArgumentException(
    ...
    private static List<String> populateWordList(String lang) {
        InputStream inputStream =
                Thread.currentThread()
                        ...
                        .getResourceAsStream(lang + "-mnemonic-word-list.txt");
        try {
