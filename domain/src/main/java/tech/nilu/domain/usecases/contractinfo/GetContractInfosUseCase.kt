...
class GetContractInfosUseCase @Inject constructor(
    private val repository: ContractInfoRepository,
    dispatcher: Dispatcher
) : UseCase<Long, List<ContractObject>>(dispatcher.io) {

    override suspend fun execute(params: Long): List<ContractObject> = repository.getContractInfos(walletId = params)
}
